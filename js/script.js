document.addEventListener('DOMContentLoaded', function() {
    ...
    addToCartButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            const itemPrice = parseFloat(itemPriceText.replace(/[^0-9.]/g, '')); // Strip out all non-numeric characters

            if (!isNaN(itemPrice)) { // Check if itemPrice is a valid number
                addItemToCart(itemName, itemPrice);
            } else {
                console.error('Invalid price format:', itemPriceText);
                alert('There was an error adding the item. Please check the price format.');
            }
        });
    });
